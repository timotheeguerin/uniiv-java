...
})(jQuery);(function(a){if(typeof define==="function"&&define.amd){define(["jquery"],a)
}else{a(jQuery)
}}(function(d){var a=(function(){return{extend:function(f,e){return d.extend(f,e)
},addEvent:function(f,e,g){if(f.addEventListener){f.addEventListener(e,g,false)
}else{if(f.attachEvent){f.attachEvent("on"+e,g)
}else{throw new Error("Browser doesn't support addEventListener or attachEvent")
}}},removeEvent:function(f,e,g){if(f.removeEventListener){f.removeEventListener(e,g,false)
}else{if(f.detachEvent){f.detachEvent("on"+e,g)
}}},createNode:function(e){var f=document.createElement("div");
f.innerHTML=e;
return f.firstChild
}}
}()),c={ESC:27,TAB:9,RETURN:13,UP:38,DOWN:40};
function b(f,e){var h=function(){},g=this,i={autoSelectFirst:false,appendTo:"body",serviceUrl:null,lookup:null,onSelect:null,width:"auto",minChars:1,maxHeight:300,deferRequestBy:0,params:{},formatResult:b.formatResult,delimiter:null,zIndex:9999,type:"GET",noCache:false,onSearchStart:h,onSearchComplete:h,containerClass:"autocomplete-suggestions",tabDisabled:false,dataType:"text",lookupFilter:function(k,j,l){return k.value.toLowerCase().indexOf(l)!==-1
},paramName:"query",transformResult:function(j){return j.suggestions
}};
g.element=f;
g.el=d(f);
g.suggestions=[];
...
d.Autocomplete=b;
b.formatResult=function(e,g){var f=new RegExp("(\\"+["/",".","*","+","?","|","(",")","[","]","{","}","\\"].join("|\\")+")","g"),h="("+g.replace(f,"\\$1")+")";
return e.value.replace(new RegExp(h,"gi"),"<strong>$1</strong>")
};
b.prototype={killerFn:null,initialize:function(){var h=this,i="."+h.classes.suggestion,g=h.classes.selected,f=h.options,e;
h.element.setAttribute("autocomplete","off");
h.killerFn=function(j){if(d(j.target).closest("."+h.options.containerClass).length===0){h.killSuggestions();
h.disableKillerFn()
}};
if(!f.width||f.width==="auto"){f.width=h.el.outerWidth()
...
e.on("mouseover",i,function(){h.activate(d(this).data("index"))
});
e.on("mouseout",function(){h.selectedIndex=-1;
e.children("."+g).removeClass(g)
});
e.on("click",i,function(){h.select(d(this).data("index"),false)
});
h.fixPosition();
if(window.opera){h.el.on("keypress",function(j){h.onKeyPress(j)
})
}else{h.el.on("keydown",function(j){h.onKeyPress(j)
})
}h.el.on("keyup",function(j){h.onKeyUp(j)
});
h.el.on("blur",function(){h.onBlur()
});
h.el.on("focus",function(){h.fixPosition()
})
...
},fixPosition:function(){var e=this,f;
if(e.options.appendTo!=="body"){return
}f=e.el.offset();
d(e.suggestionsContainer).css({top:(f.top+e.el.outerHeight())+"px",left:f.left+"px"})
...
if(!f.disabled&&!f.visible&&g.keyCode===c.DOWN&&f.currentValue){f.suggest();
return
}if(f.disabled||!f.visible){return
}switch(g.keyCode){case c.ESC:f.el.val(f.currentValue);
f.hide();
break;
case c.TAB:case c.RETURN:if(f.selectedIndex===-1){f.hide();
return
}f.select(f.selectedIndex,g.keyCode===c.RETURN);
if(g.keyCode===c.TAB&&this.options.tabDisabled===false){return
}break;
case c.UP:f.moveUp();
break;
...
}switch(g.keyCode){case c.UP:case c.DOWN:return
}clearInterval(f.onChangeInterval);
if(f.currentValue!==f.el.val()){if(f.options.deferRequestBy>0){f.onChangeInterval=setInterval(function(){f.onValueChange()
},f.options.deferRequestBy)
}else{f.onValueChange()
}}},onValueChange:function(){var e=this,f;
clearInterval(e.onChangeInterval);
e.currentValue=e.element.value;
f=e.getQuery(e.currentValue);
e.selectedIndex=-1;
if(e.ignoreValueChange){e.ignoreValueChange=false;
return
}if(f.length<e.options.minChars){e.hide()
}else{e.getSuggestions(f)
}},getQuery:function(f){var e=this.options.delimiter,g;
if(!e){return d.trim(f)
}g=f.split(e);
return d.trim(g[g.length-1])
},getSuggestionsLocal:function(h){var g=this,e=h.toLowerCase(),f=g.options.lookupFilter;
return{suggestions:d.grep(g.options.lookup,function(i){return f(i,h,e)
})}
},getSuggestions:function(h){var e,g=this,f=g.options;
e=g.isLocal?g.getSuggestionsLocal(h):g.cachedResponse[h];
if(e&&d.isArray(e.suggestions)){g.suggestions=e.suggestions;
g.suggest()
}else{if(!g.isBadQuery(h)){f.onSearchStart.call(g.element,h);
f.params[f.paramName]=h;
d.ajax({url:f.serviceUrl,data:f.params,type:f.type,dataType:f.dataType}).done(function(i){g.processResponse(i);
f.onSearchComplete.call(g.element,h)
})
}}},isBadQuery:function(f){var g=this.badQueries,e=g.length;
...
return
}var i=this,k=i.options.formatResult,j=i.getQuery(i.currentValue),h=i.classes.suggestion,g=i.classes.selected,e=d(i.suggestionsContainer),f="";
d.each(i.suggestions,function(m,l){f+='<div class="'+h+'" data-index="'+m+'">'+k(l,j)+"</div>"
});
e.html(f).show();
i.visible=true;
if(i.options.autoSelectFirst){i.selectedIndex=0;
...
},processResponse:function(g){var f=this,e=typeof g=="string"?d.parseJSON(g):g;
e.suggestions=f.verifySuggestionsFormat(f.options.transformResult(e));
if(!f.options.noCache){f.cachedResponse[e[f.options.paramName]]=e;
if(e.suggestions.length===0){f.badQueries.push(e[f.options.paramName])
}}if(e[f.options.paramName]===f.getQuery(f.currentValue)){f.suggestions=e.suggestions;
f.suggest()
}},activate:function(f){var i=this,j,h=i.classes.selected,e=d(i.suggestionsContainer),g=e.children();
...
},select:function(g,e){var h=this,f=h.suggestions[g];
if(f){h.el.val(f);
...
},adjustScroll:function(e){var g=this,k=g.activate(e),f,i,j,h=25;
if(!k){return
}f=k.offsetTop;
...
d.fn.autocomplete=function(f,e){return this.each(function(){var i="autocomplete",h=d(this),g;
if(typeof f==="string"){g=h.data(i);
if(typeof g[f]==="function"){g[f](e)
}}else{g=new b(this,f);
h.data(i,g)
}})
}
...
}));(function(a){a.fn.textfill=function(d){var f={debug:false,maxFontPixels:40,minFontPixels:4,innerTag:"span",widthOnly:false,callback:null,complete:null,explicitWidth:null,explicitHeight:null};
var c=jQuery.extend(f,d);
function b(l,m,k,j,h,i){if(!c.debug){return
}function g(p,o){var n=" / ";
if(p>o){n=" > "
}else{if(p==o){n=" = "
}}return n
}console.debug(l+"font: "+m.css("font-size")+", H: "+m.height()+g(m.height(),k)+k+", W: "+m.width()+g(m.width(),j)+j+", minFontPixels: "+h+", maxFontPixels: "+i)
}function e(j,i,g,l,m,n,k,h){b(j+": ",i,m,n,k,h);
while(k<h-1){var o=Math.floor((k+h)/2);
i.css("font-size",o);
if(g.call(i)<=l){k=o;
if(g.call(i)==l){break
}}else{h=o
...
}this.each(function(){var k=a(c.innerTag+":visible:first",this);
var m=c.explicitHeight||a(this).height();
var n=c.explicitWidth||a(this).width();
var i=k.css("font-size");
var o;
if(c.debug){console.log("Opts: ",c);
console.log("Vars: maxHeight: "+m+", maxWidth: "+n)
}var l=c.minFontPixels;
var h=c.maxFontPixels<=0?m:c.maxFontPixels;
var g=undefined;
if(!c.widthOnly){g=e("H",k,a.fn.height,m,m,n,l,h)
}var j=e("W",k,a.fn.width,n,m,n,l,h);
if(c.widthOnly){k.css("font-size",j)
}else{k.css("font-size",Math.min(g,j))
}if(c.debug){console.debug("Final: "+k.css("font-size"))
}if(k.width()>n||k.height()>m){k.css("font-size",i)
}if(c.callback){c.callback(this)
}});
if(c.complete){c.complete(this)
}return this
}
})(jQuery);/*
 ...
 */
(function(d,c,a,e){var b={init:function(f){var g=d.extend({image:null,width:50,height:50,disabled:false},f);
return this.each(function(){var l=d(this);
var k=g.image;
var i=l.data("sdb-image");
if(i){k=i
}if(!k){d.error("There is no image assigned for ScrewDefaultButtons")
}l.wrap("<div >").css({display:"none"});
var q=l.attr("class");
var j=l.attr("onclick");
var n=l.parent("div");
n.addClass(q);
n.attr("onclick",j);
n.css({"background-image":k,width:g.width,height:g.height,cursor:"pointer"});
var m=0;
var p=-(g.height);
if(l.is(":disabled")){m=-(g.height*2);
p=-(g.height*3)
}l.on("disableBtn",function(){l.attr("disabled","disabled");
m=-(g.height*2);
p=-(g.height*3);
l.trigger("resetBackground")
});
l.on("enableBtn",function(){l.removeAttr("disabled");
m=0;
p=-(g.height);
l.trigger("resetBackground")
});
l.on("resetBackground",function(){if(l.is(":checked")){n.css({backgroundPosition:"0 "+p+"px"})
}else{n.css({backgroundPosition:"0 "+m+"px"})
}});
l.trigger("resetBackground");
if(l.is(":checkbox")){n.on("click",function(){if(!(l.is(":disabled"))){l.change()
}});
n.addClass("styledCheckbox");
l.on("change",function(){if(l.prop("checked")){l.prop("checked",false);
n.css({backgroundPosition:"0 "+m+"px"})
}else{l.prop("checked",true);
n.css({backgroundPosition:"0 "+p+"px"})
}})
}else{if(l.is(":radio")){n.addClass("styledRadio");
var o=l.attr("name");
n.on("click",function(){if(!(l.prop("checked"))&&!(l.is(":disabled"))){l.change()
}});
l.on("change",function(){if(l.prop("checked")){l.prop("checked",false);
n.css({backgroundPosition:"0 "+m+"px"})
}else{l.prop("checked",true);
n.css({backgroundPosition:"0 "+p+"px"});
var s=d('input[name="'+o+'"]').not(l);
s.trigger("radioSwitch")
}});
l.on("radioSwitch",function(){n.css({backgroundPosition:"0 "+m+"px"})
});
var r=d(this).attr("id");
var h=d('label[for="'+r+'"]');
h.on("click",function(){n.trigger("click")
})
}}if(!d.support.leadingWhitespace){var r=d(this).attr("id");
var h=d('label[for="'+r+'"]');
h.on("click",function(){n.trigger("click")
})
}})
},check:function(){return this.each(function(){var f=d(this);
if(!b.isChecked(f)){f.change()
}})
},uncheck:function(){return this.each(function(){var f=d(this);
if(b.isChecked(f)){f.change()
}})
},toggle:function(){return this.each(function(){var f=d(this);
f.change()
})
},disable:function(){return this.each(function(){var f=d(this);
f.trigger("disableBtn")
})
},enable:function(){return this.each(function(){var f=d(this);
f.trigger("enableBtn")
})
},isChecked:function(f){if(f.prop("checked")){return true
}return false
}};
d.fn.screwDefaultButtons=function(g,f){if(b[g]){return b[g].apply(this,Array.prototype.slice.call(arguments,1))
}else{if(typeof g==="object"||!g){return b.init.apply(this,arguments)
}else{d.error("Method "+g+" does not exist on jQuery.screwDefaultButtons")
}}};
return this
})(jQuery);