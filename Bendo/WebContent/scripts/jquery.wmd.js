...
(function() {
	...
	WMDEditor.defaults = { // {{{
		...
		// entering links.
		imageDialogText : "<p style='margin-top: 0px'><b>Enter the image URL.</b></p>"
		...
		linkDialogText : "<p style='margin-top: 0px'><b>Enter the web address.</b></p>"
				+ "<p>You can also add a title, which will be displayed as a tool tip.</p>"
		...
		// Some intervals in ms. These can be adjusted to reduce the control's
		// load.
	...
	WMDEditor.prototype = {
		getPanels : function() {
			var preview;
			if (typeof this.options.preview == "string" || this.options.preview instanceof String) {
	...
	var util = { // {{{
		...
		// Adds a listener callback to a DOM element which is fired on a
		// specified
		// event.
		...
		// Removes a listener callback from a DOM element which is fired on a
		// specified
		// event.
		...
		extendRegExp : function(regex, pre, post) {
			...
			// Remove the flags and slash delimiters from the regular
			// expression.
		...
		// makeLinkMarkdown: The function which is executed when the prompt is
		// dismissed, either via OK or Cancel
		prompt : function(text, defaultInputText, makeLinkMarkdown) {

			// These variables need to be declared at this level since they are
			// used
			...
			var createDialog = function() {
				...
				// This has to be done AFTER adding the dialog to the form if
				// you
	...
	Chunks.prototype.addBlankLines = function(nLinesBefore, nLinesAfter, findExtraNewlines) {
		...
		// New bug discovered in Chrome, which appears to be related to use of
		// RegExp.$1
		// Hack it to hold the match results. Sucks because we're double
		// matching...
		match = /(^\n*)/.exec(this.selection);
	...
	var InputPoller = function(textarea, callback, interval) { // {{{
		...
		// Stored start, end and text. Used to see if there are changes to the
		// input.
		var lastStart;
		var lastEnd;
		var markdown;
		...
		this.tick = function() {
			...
			if (inputArea.selectionStart || inputArea.selectionStart === 0) {
				...
				if (inputArea.value == "") {
					...
					// value in code, selectionStart and selectionEnd don't
					// change.
					start = end = 0;
				} else {
	...
	var PreviewManager = function(wmd) { // {{{
		...
		// Processes our math syntax before giving the text to Markdown
		// processing
		...
		var makePreviewHtml = function() {
			...
			if (code_tag.length > 0) {
				code_tag.replaceWith(function() {
					return $("<code>" + $(this).html().replace(/\\\\([\(\[])([\s\S]*?)\\\\([\]\)])/g, function(match) {
		...
		// Refreshes the preview area. Calls user supplied callbacks if
		// provided.
		...
		var pushPreviewHtml = function(text) {
			...
			if (wmd.panels.output) {
				// The value property is only defined if the output is a
				// textarea.
	...
	var UndoManager = function(textarea, pastePollInterval, callback) { // {{{
		...
		this.undo = function() {
			...
			if (undoObj.canUndo()) {
				if (lastState) {
					// What about setting state -1 to null or checking for
					// undefined?
					lastState.restore();
					lastState = null;
				} else {
	...
	var wmdBase = function(wmd, wmd_options) { // {{{
		...
		// I think my understanding of how the buttons and callbacks are stored
		// in the array is incomplete.
		wmd.editor = function(previewRefreshCallback) { // {{{
			...
			var setupButton = function(button, isEnabled) {
				...
				if (isEnabled) {
					...
					// IE tries to select the background image "button" text
					// (it's
					// implemented in a list item) so we have to cache the
					// selection
					// on mousedown.
					if (browser.isIE) {
			...
			var setupEditor = function() {
				...
				util.addEvent(inputBox, keyEvent, function(key) {
					...
					// Check to see if we have a button key and, if so execute
					// the callback.
		...
		command.doBorI = function(chunk, nStars, insertText) {
			...
			if ((prevStars >= nStars) && (prevStars != 2 || nStars != 1)) {
				...
				// It's not really clear why this code is necessary. It just
				// moves
				...
				// In most cases, if you don't have any selected text and click
				// the button
				// you'll get a selected, marked up region with the default text
				// inserted.
		...
		command.addLinkDef = function(chunk, linkDef) {
			...
			var defsToAdd = {}; //
			// Start with a clean slate by removing all previous link
			// definitions.
		...
		command.doLinkOrImage = function(chunk, postProcessing, isImage) {
			...
			if (chunk.endTag.length > 1) {
				...
				// The function to be executed when you enter a link and press
				// OK or Cancel.
		...
		// Moves the cursor to the next line and continues lists, quotes and
		// code.
		...
		command.doList = function(chunk, postProcessing, isNumberedList, useDefaultText) {
			...
			// Should probably use the regex extension function to make this
			// clearer.
			...
			// Get the item prefix - e.g. " 1. " for a numbered list, " - " for
			// a bulleted list.
			...
			if (chunk.startTag) {
				...
				if (hasDigits) {
					// Have to renumber the bullet points if this is a numbered
					// list.
		...
		command.doHeading = function(chunk, postProcessing, useDefaultText) {
			...
			// Try to get the current header level by looking for - and = in the
			// line
			// below the selection.
			...
			// If there is a header level, we substract one from the header
			// level.
			...
			if (headerLevelToCreate > 0) {
				...
				// Why not have it iterate over hash header levels? Wouldn't
				// that be easier and cleaner?
...
Showdown.converter = function() {
	...
	this.makeHtml = function(text) {
		...
		// _EscapeSpecialCharsWithinTagAttributes(), so that any *'s or _'s in
		// the <a>
	...
	var _StripLinkDefinitions = function(text) {
		...
		/*
		 * var text = text.replace(/ ^[ ]{0,3}\[(.+)\]: // id = $1 attacklab:
		 * g_tab_width - 1 [ \t]* \n? // maybe *one* newline [ \t]* <?(\S+?)>? //
		 * url = $2 [ \t]* \n? // maybe one newline [ \t]* (?: (\n*) // any
		 * lines skipped = $3 attacklab: lookbehind removed ["(] (.+?) // title =
		 * $4 [")] [ \t]* )? // title is optional (?:\n+|$) /gm,
		 * function(){...});
		 */
		var text = text.replace(/^[ ]{0,3}\[(.+)\]:[ \t]*\n?[ \t]*<?(\S+?)>?[ \t]*\n?[ \t]*(?:(\n*)["(](.+?)[")][ \t]*)?(?:\n+|\Z)/gm, function(wholeMatch, m1,
				m2, m3, m4) {
			m1 = m1.toLowerCase();
	...
	var _HashHTMLBlocks = function(text) {
		...
		/*
		 * var text = text.replace(/ ( // save in $1 ^ // start of line (with
		 * /m) <($block_tags_a) // start tag = $2 \b // word break // attacklab:
		 * hack around khtml/pcre bug... [^\r]*?\n // any number of lines,
		 * minimally matching </\2> // the matching end tag [ \t]* // trailing
		 * spaces/tabs (?=\n+) // followed by a newline ) // attacklab: there
		 * are sentinel newlines at end of document /gm,function(){...}};
		 */
		text = text.replace(
		...
		/*
		 * var text = text.replace(/ ( // save in $1 ^ // start of line (with
		 * /m) <($block_tags_b) // start tag = $2 \b // word break // attacklab:
		 * hack around khtml/pcre bug... [^\r]*? // any number of lines,
		 * minimally matching .*</\2> // the matching end tag [ \t]* //
		 * trailing spaces/tabs (?=\n+) // followed by a newline ) // attacklab:
		 * there are sentinel newlines at end of document /gm,function(){...}};
		 */
		 ...
		 * line [ ]{0,3} (<(hr) // start tag = $2 \b // word break ([^<>])*? //
		 * \/?>) // the matching end tag [ \t]* (?=\n{2,}) // followed by a
		 * blank line ) /g,hashElement);
		 */
		text = text.replace(/(\n[ ]{0,3}(<(hr)\b([^<>])*?\/?>)[ \t]*(?=\n{2,}))/g, hashElement);
		...
		/*
		 * text = text.replace(/ ( // save in $1 \n\n // Starting after a blank
		 * line [ ]{0,3} // attacklab: g_tab_width - 1 <! (--[^\r]*?--\s*)+ > [
		 ...
		 * save in $1 [ ]{0,3} // attacklab: g_tab_width - 1 (?: <([?%]) // $2
	...
	var _DoAnchors = function(text) {
		 ...
		 * text = text.replace(/ ( // wrap whole match in $1 \[ ( (?: \[[^\]]*\] //
		 * allow brackets nested one level | [^\[\]] // or anything else ) ) \] \( //
		 * literal paren [ \t]* () // no id, so leave $3 empty <?(.*?)>? // href =
		 * $4 [ \t]* ( // $5 (['"]) // quote char = $6 (.*?) // Title = $7 \6 //
		 * matching quote [ \t]* // ignore any spaces/tabs between closing quote
		 * and ) )? // title is optional \) ) /g,writeAnchorTag);
		 */
		...
		/*
		 * text = text.replace(/ ( // wrap whole match in $1 \[ ([^\[\]]+) //
		 * link text = $2; can't contain '[' or ']' \] )()()()()() // pad rest
		 * of backreferences /g, writeAnchorTag);
		 */
	...
	var _DoImages = function(text) {
		...
		/*
		 * text = text.replace(/ ( // wrap whole match in $1 !\[ (.*?) // alt
		 * text = $2 \] \s? // One optional whitespace character \( // literal
		 * paren [ \t]* () // no id, so leave $3 empty <?(\S+?)>? // src url =
		 * $4 [ \t]* ( // $5 (['"]) // quote char = $6 (.*?) // title = $7 \6 //
		 * matching quote [ \t]* )? // title is optional \) ) /g,writeImageTag);
		 */
	...
	var _DoHeaders = function(text) {
		...
		/*
		 * text = text.replace(/ ^(\#{1,6}) // $1 = string of #'s [ \t]* (.+?) //
	...
	var _DoLists = function(text) {
		...
		/*
		 * var whole_list = / ( // $1 = whole list ( // $2 [ ]{0,3} //
		 * attacklab: g_tab_width - 1 ([*+-]|\d+[.]) // $3 = first list item
		 * marker [ \t]+ ) [^\r]+? ( // $4 ~0 // sentinel for workaround; should
		 * be $ | \n{2,} (?=\S) (?! // Negative lookahead for another list item
		 * marker [ \t]* (?:[*+-]|\d+[.])[ \t]+ ) ) )/g
		 */
	...
	_ProcessListItems = function(list_str) {
		...
		/*
		 * list_str = list_str.replace(/ (\n)? // leading line = $1 (^[ \t]*) //
		 * leading whitespace = $2 ([*+-]|\d+[.]) [ \t]+ // list marker = $3
		 * ([^\r]+? // list item text = $4 (\n{1,2})) (?= \n* (~0 | \2
		 * ([*+-]|\d+[.]) [ \t]+)) /gm, function(){...});
		 */
		list_str = list_str.replace(/(\n)?(^[ \t]*)([*+-]|\d+[.])[ \t]+([^\r]+?(\n{1,2}))(?=\n*(~0|\2([*+-]|\d+[.])[ \t]+))/gm, function(wholeMatch, m1, m2,
				m3, m4) {
			var item = m4;
			var leading_line = m1;
	...
	var _DoCodeBlocks = function(text) {
		 ...
		 * or more lines, starting with a space/tab (?: (?:[ ]{4}|\t) // Lines
		 * must start with a tab or a tab-width of spaces - attacklab:
		 * g_tab_width .*\n+ )+ ) (\n*[ ]{0,3}[^ \t\n]|(?=~0)) // attacklab:
		 * g_tab_width /g,function(){...});
		 */

		// attacklab: sentinel workarounds for lack of \A and \Z, safari\khtml
		// bug
		text += "~0";
	...
	var _DoCodeSpans = function(text) {
		...
		/*
		 * text = text.replace(/ (^|[^\\]) // Character before opening ` can't
		 * be a backslash (`+) // $2 = Opening run of ` ( // $3 = The code block
		 * [^\r]*? [^`] // attacklab: work around lack of lookbehind ) \2 //
	...
	var _DoAutoLinks = function(text) {
		...
		/*
		 * text = text.replace(/ < (?:mailto:)? ( [-.\w]+ \@
		 * [-a-z0-9]+(\.[-a-z0-9]+)*\.[a-z]+ ) > /gi, _DoAutoLinks_callback());
		 */
		...
		/* wiki links */
		// text = text.replace(/(^| )(\b[A-Z][a-z]+[A-Z][\w\u00A1-\uFFFF]+\b)/,
		// "<a href=\""+site.url+"/$1\">$1</a>");
