...
(function($) {
	...
	$.fn.noUiSlider = function(options, flag) {
		...
		function client(f) {
			try {
				return [ (f.clientX || f.originalEvent.clientX || f.originalEvent.touches[0].clientX),
			...
			create : function() {
				...
				return this
						.each(function() {
							...
							split = (settings.serialization.resolution = settings.serialization.resolution || 0.01).toString().split('.'), res = split[0] == 1 ? 0
									: split[1].length;
							...
							for ( var i = 0; i < settings.handles; i++) {
								...
								var bind = '.noUiSlider', onEvent = (EVENT === 1 ? 'mousedown' : EVENT === 2 ? 'MSPointerDown' : 'touchstart') + bind + 'X', moveEvent = (EVENT === 1 ? 'mousemove'
										: EVENT === 2 ? 'MSPointerMove' : 'touchmove')
										+ bind, offEvent = (EVENT === 1 ? 'mouseup' : EVENT === 2 ? 'MSPointerUp' : 'touchend') + bind
														...
														$(document)
																.on(
																		...
																		function(f) {
																			...
																			var movement = [ previousClick[0] != currentClick[0],
																					previousClick[1] != currentClick[1] ], proposal = originalPosition
																					+ ((currentClick[orientation] * 100) / (orientation ? slider.height()
																							: slider.width()));
																			proposal = correct(proposal, slider, handle);

																			if (movement[orientation] && proposal != previousProposal) {
																				handle.css(pos, proposal + '%').data('input').val(
																						percentage.is(settings.range, proposal).toFixed(res));
																				call(settings.slide, slider.data('_n', true));
																				previousProposal = proposal;
																				handle.css('z-index', handles.length == 2 && proposal == 100
																						&& handle.is(':first-child') ? 2 : 1);
																			}
							...
							if (EVENT == 1) {
								slider
										.on(
												...
												function(f) {
													if (!slider.hasClass('disabled')) {
														var currentClick = client(f), proposal = ((currentClick[orientation] - slider.offset()[pos]) * 100)
																/ (orientation ? slider.height() : slider.width()), handle = handles.length > 1 ? (currentClick[orientation] < (handles[0]
																.offset()[pos] + handles[1].offset()[pos]) / 2 ? handles[0] : handles[1])
																: handles[0];
							...
							for ( var i = 0; i < handles.length; i++) {
								...
								if (typeof settings.serialization.to[i] == 'string') {
									handles[i].data('input', slider.append('<input type="hidden" name="' + settings.serialization.to[i] + '">').find(
											'input:last').val(val).change(function(a) {
										a.stopPropagation();
									}));
					...
					return this.each(function() {
						...
						for ( var i = 0; i < setup.handles.length; i++) {
							if (val[i] != null) {
								...
								setup.handles[i].css(setup.pos, proposal + '%').data('input').val(
										percentage.is(setup.settings.range, proposal).toFixed(setup.res));
							}
						}
					});
