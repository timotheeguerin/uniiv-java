...
if (window.jQuery)
	(function($) {
		...
		$.fn.rating = function(options) {
			...
			this.not('.star-rating-applied').addClass('star-rating-applied').each(
					function() {
						...
						// if rater is available, verify that the control still
						// exists
						if (rater)
							control = rater.data('rating');

						if (rater && control)// {// save a byte!
							// add star to control if rater is available and the
							// same control still exists
							control.count++;

						// }// save a byte!
						else {
							// create new control if first star or control
							// element was removed/replaced
							...
							// Mark element for initialization (once all stars
							// are ready)
							...
							// Accept required setting from class property
							// (class='required')
							...
							rater.append(control.cancel = $(
									'<div class="rating-cancel"><a title="' + control.cancel + '">' + control.cancelValue + '</a></div>').on('mouseover',
									function() {
										$(this).rating('drain');
						...
						// insert rating star (thanks Jan Fanslau rev125 for
						// blind support
						// https://code.google.com/p/jquery-star-rating-plugin/issues/detail?id=125)
						var star = $('<div role="text" aria-label="' + this.title + '" class="star-rating rater-' + control.serial + '"><a title="'
						...
						if (typeof control.split == 'number' && control.split > 0) {
							...
							star
							// restrict star's width and hide overflow (already
							// in CSS)
							.width(spw)
						...
						if (control.readOnly)// { //save a byte!
							// Mark star as readOnly so user can customize
							// display
						...
						// store control information in form (or body when form
						// not available)
		...
		$.extend($.fn.rating, {
			...
			// each time the plugin is invoked so same name controls can
			// co-exist
			calls : 0,
				...
				if (typeof value != 'undefined' || this.length > 1) {
					...
					if (typeof value == 'string') {
						...
						$.each(control.stars, function() {
							// console.log($(this).data('rating.input'),
							// $(this).data('rating.input').val(), value,
							// $(this).data('rating.input').val()==value?'BINGO!':'');
		...
		// be.one('change', //focus: function(){}, // executed when stars are
		// focused
